...
# A simple ODE example: dx/dt = -0.5 * x
def model(t, x):
    return -0.5 * x

def benchmark_ode_solver():
    sizes = [1000, 5000, 10000, 20000]  # Different number of time steps for ODE solving
    cpu_times = []
    gpu_times = []
    ...
    for size in sizes:
        ...
        t = np.linspace(0, 10, size)  # Time range

        # CPU Benchmark (using odeint)
        start_time = time.time()
        result_cpu = odeint(model, y0, t)
        cpu_times.append(time.time() - start_time)

        # GPU Benchmark (using odeint_wrapper function)
        start_time = time.time()
